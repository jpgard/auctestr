...
#'
#' @references Stouffer, S.A.; Suchman, E.A.; DeVinney, L.C.; Star, S.A.; Williams, R.M. Jr. (1949). The American Soldier, Vol.1: Adjustment during Army Life.)
...
#' @param filter_value (optional) keep only observations which contain \code{filter_value} for \code{filter_col}.
#' @param time_col name of column in df representing time of observations (z-scores are averaged over time_col within each model/dataset due to non-independence). These can also be other dependent groupings, such as cross-validation folds.
#' @param outcome_col name of column in df representing outcome to compare; this should be Area Under the Receiver Operating Characteristic or A' statistic (this method applies specifically to AUC and not other metrics (i.e., sensitivity, precision, F1)..
#' @param compare_col name of column in df representing two conditions to compare (should contain at least 2 unique values; these two values are specified as \code{compare_values}).
#' @param over_col identifier for independent experiments, iterations, etc. over which z-scores for models are to be compared (using Stouffer's Z; see  Stouffer, S.A.; Suchman, E.A.; DeVinney, L.C.; Star, S.A.; Williams, R.M. Jr. (1949). The American Soldier, Vol.1: Adjustment during Army Life.)
#' @param n_col name of column in df with total number of observations in the sample tested by each row.
...
fbh_auc_compare <- function(df, compare_values, filter_value, time_col = "time", outcome_col = "auc", compare_col = "model_id", over_col = "dataset", n_col = "n", n_p_col = "n_p", n_n_col = "n_n", filter_col = "model_variant"){
    ...
    for (dataset_ix in seq_along(unique(comp_df[,over_col]))){
        ...
        message(paste0("fetching comparison results for models ", compare_values[1], ", ", compare_values[2], " in dataset ", dataset, " with filter value ", filter_value))
